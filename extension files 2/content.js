...
// Function to scrape and filter the list
function scrapeAndFilterList() {
    ...
    let ingredients = document.querySelectorAll('.css-26us6o');
    let filteredItems = ingredients.filter(ingredient => allergens.includes(ingredient.toLowerCase()));
...
chrome.runtime.onMessage.addListener(function(request, sender, sendResponse) {
    if (request.action === 'scrapeAndFilterList') {
        let filteredList = scrapeAndFilterList();
        sendResponse({ filteredList: filteredList });
    }
});
